...
@Component
public class RepairCategoryRepositorySQL extends SqlAnnotationDrivenRepository<RepairCategory> implements RepairCategoryRepository {


    public static final String FIND_BY_KEY_NAME_SQL = "SELECT rc.id, rc.key_name, rw.id, rw.key_name ,rw.category_id FROM repair_categories AS rc LEFT JOIN repair_works AS rw ON rw.category_id = rc.id WHERE rc.key_name = ?";
    private static final Logger log = org.slf4j.LoggerFactory.getLogger(RepairCategoryRepositorySQL.class);

    @Autowire
    public RepairCategoryRepositorySQL(JdbcTemplate template, SqlEntityMapper entityMapper, SqlEntityQueryGenerator queryGenerator, AnnotationSqlDefinitionReader definitionReader) {
    ...
    public List<RepairCategory> findAll(Sort sort) {
        ...
        for (RepairCategory category : categories) {
            List<RepairWork> list = template.query(queryGenerator.selectQuery(RepairWork.class) + " where category_id = " + category.getId(), (rs, rowNum) -> entityMapper.map(rs, RepairWork.class));
    ...
    @Override
    public Optional<RepairCategoryLocalPart> findFirstLocalPartByCategory_IdAndLanguage_Lang(Long categoryId, String lang) {
        log.trace("Lang {}",lang);
        return template.query("SELECT lp.* FROM repair_category_local_parts as lp LEFT JOIN app_locales as l on lp.language_id = l.id where lp.category_id = ? and l.lang = ?", pss -> {
